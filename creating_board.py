...
def draw_insects(surface, color):
    ...
    y_distance = int((surface_height-4*hexa_size*3**(1/2))/5)
    ant = hs.hexagon_stone(hexa_size, surface, "ant", color)
    hopper = hs.hexagon_stone(hexa_size , surface, "hopper", color)
    spider = hs.hexagon_stone(hexa_size , surface, "spider", color)
    bee = hs.hexagon_stone(hexa_size, surface, "bee", color)
    
    ant.draw_stone((frame_x_size/4, y_distance))
    hopper.draw_stone((frame_x_size/4, 2*y_distance + hexa_size*3**(1/2)))
    spider.draw_stone((frame_x_size/4, 3*y_distance + 2*hexa_size*3**(1/2)))
    bee.draw_stone((frame_x_size/4, 4*y_distance + 3 * hexa_size*3**(1/2)))
